...
def prepare_image(root_dir, viz_root_dir, fn1, fn2, keep_size):
    ...
    viz_fn = osp.join(viz_dir, filename + '.jpg')
...
def visualize_flow(args, seq_dir, root_dir, viz_root_dir, model, img_pairs, keep_size):
    weights = None
    idx = 1
    out_dir = os.path.join('D:/bdd100k/flows/train/', args.name)
    ...
    for img_pair in img_pairs:
        ...
        output_file = os.path.join(out_dir,'%06d.flo' % (idx))
...
def generate_pairs(dirname, start_idx, end_idx):
    ...
    for idx in range(start_idx, end_idx):
        img1 = osp.join(dirname, f'{idx:06}.jpg')
        img2 = osp.join(dirname, f'{idx+1:06}.jpg')
        # img1 = f'{idx:06}.png'
        # img2 = f'{idx+1:06}.png'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='aaa')
    ...
    with torch.no_grad():
        visualize_flow(args, args.seq_dir, root_dir, viz_root_dir, model, img_pairs, args.keep_size)